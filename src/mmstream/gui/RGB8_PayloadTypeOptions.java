...
public class RGB8_PayloadTypeOptions implements PayloadTypeOptions{

public RGB8_PayloadTypeOptions() {
  panelbuilt = false;
  Options = null;
}
...
public Panel getPanel() {
  ...
  Label l = new Label("RGB 8bit Options:", Label.CENTER);
...
public PayloadType createPayloadType(AppManager am)     throws Gui_Exception {
  if (panelbuilt == false) 
    throw new Gui_Exception("RGB8_PayloadTypeOptions.createPayloadType():not initialised");

  Type pt = null;
//   pt = SessionMapper.payloadTypeHandler.queryType(Name);
  pt = am.getPayloadTypeHandlerTable().queryType(Name);
  if (pt == null) 
    throw new Gui_Exception("RGB8_PayloadTypeOptions.createPayload(): no Type for "+Name);
  ...
  try {
    ...
    throw new Gui_Exception("RGB8_PayloadTypeOptions.createPayloadType():  Integer.parseInt():: "+ne.getMessage());
  }
  byte[] ret = new byte[9];
  int off = 0;
  for (int i = 3; i >= 0; i--)
  ...
  PayloadType retpt = new PayloadType(pt.getName(), pt.getByteCode(), ret, 1, w*h, w*h);
...
public static final String Name = new String(Payload_TypeConfiguration.PT_RGB8_Name);
}
